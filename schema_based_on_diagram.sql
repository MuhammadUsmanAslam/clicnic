...
CREATE TABLE invoices (id INT GENERATED ALWAYS AS IDENTITY,total_amount NUMERIC(5,2),generated_at TIMESTAMP,payed_at TIMESTAMP,medical_histoy_id INT DEFAULT 0,PRIMARY KEY(id),UNIQUE(medical_histoy_id),CONSTRAINT fk_medical_histoy FOREIGN KEY(medical_histoy_id) REFERENCES medical_histories(id));
...
CREATE INDEX treatment_histories_treatment_id_asc ON treatment_histories(treatment_id ASC);